...
class DepreciationsReportTransformer
{
    ...
    public function transformDepreciations (Collection $depreciations)
    {
        ...
        $reportsArray = array();
        foreach ($depreciations as $depreciations)  {
            $reportsArray[] = self::transformDepreciationReport($depreciations);

        }
        return (new DatatablesTransformer)->transformDatatables(($reportsArray));
    }

    public function transformDepreciationReport (Asset $asset)
    {
        $array = [
