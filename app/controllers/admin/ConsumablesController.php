...
class ConsumablesController extends AdminController
{
    ...
    **/
    ...
    /**
    * Check the consumable back into inventory
    *
    * @param  int  $consumableId
    * @return View
    **/
    public function getCheckin($consumableUserId = null, $backto = null)
    {
        // Check if the consumable exists
        if (is_null($consumable_user = DB::table('consumables_users')->find($consumableUserId))) {
            // Redirect to the consumable management page with error
            return Redirect::to('admin/consumables')->with('error', Lang::get('admin/consumables/message.not_found'));
        }

		$consumable = Consumable::find($consumable_user->consumable_id);
        return View::make('backend/consumables/checkin', compact('consumable'))->with('backto',$backto);
    }
    ...
    * @return View
    **/
    public function postCheckin($consumableUserId = null, $backto = null)
    {
        // Check if the consumable exists
        if (is_null($consumable_user = DB::table('consumables_users')->find($consumableUserId))) {
            // Redirect to the consumable management page with error
		...
		$consumable = Consumable::find($consumable_user->consumable_id);
        $logaction = new Actionlog();
        $logaction->checkedout_to = $consumable_user->assigned_to;
        $return_to = $consumable_user->assigned_to;
        $admin_user = Sentry::getUser();


        // Was the consumable updated?
        if(DB::table('consumables_users')->where('id', '=', $consumable_user->id)->delete()) {
				...
				try {
						$client->attach([
						    ...
						    'fields' => [
						        [
						            'title' => 'Checked In:',
						            'value' => strtoupper($logaction->asset_type).' <'.Config::get('app.url').'/admin/consumables/'.$consumable->id.'/view'.'|'.$consumable->name.'> checked in by <'.Config::get('app.url').'/admin/users/'.$admin_user->id.'/view'.'|'.$admin_user->fullName().'>.'
						        ],
						        [
						            'title' => 'Note:',
						            'value' => e($logaction->note)
            ...
            if ($backto=='user') {
				return Redirect::to("admin/users/".$return_to.'/view')->with('success', Lang::get('admin/consumables/message.checkin.success'));
			} else {
				return Redirect::to("admin/consumables/".$consumable->id."/view")->with('success', Lang::get('admin/consumables/message.checkin.success'));
			}
        }

        // Redirect to the consumable management page with error
        return Redirect::to("admin/consumables")->with('error', Lang::get('admin/consumables/message.checkin.error'));
    }
