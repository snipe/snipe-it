    ...
    <div class="form-group{{ $errors->has($field->db_column_name()) ? ' has-error' : '' }}">
      ...
      <div class="col-md-7 col-sm-12{{ ($field->pivot->required=='1') ? ' required' : '' }}">
          ...
          @if ($field->element!='text')
              ...
              @elseif ($field->element=='checkbox' && !$field->field_encrypted)
                    <!-- Checkboxes -->
              ...
              @elseif ($field->element == 'checkbox' && $field->field_encrypted && !Gate::allows('admin'))
                  <div>
                      <input class="form-control disabled" type="text"
                             value="{{ strtoupper(trans('admin/custom_fields/general.encrypted')) }}" disabled>
                  </div>
              @elseif ($field->element == 'checkbox' && $field->field_encrypted && Gate::allows('admin'))
                  @foreach ($field->formatFieldValuesAsArray() as $key => $value)
                      <div>
