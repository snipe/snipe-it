...
<div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}"{!!  (isset($style)) ? ' style="'.e($style).'"' : ''  !!}>
    ...
    <div class="col-md-1 col-sm-1 text-left" style="margin-left: -10px;">
        @can('create', \App\Models\User::class)
            @if ((!isset($hide_new)) || ($hide_new!='true'))
            <a href='{{ route('modal.show', 'location') }}'   data-toggle="modal" data-target="#removeModal" data-select='{{ $fieldname }}_location_select' class="btn btn-sm btn-danger">Remove from Location</a>
            @endif
        @endcan
    </div>
