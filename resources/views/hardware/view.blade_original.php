...
<div class="row">
  ...
  <div class="col-md-12">
    ...
    @if (($asset->assetstatus) && ($asset->assetstatus->deployable=='1'))
      @if ($asset->assigned_to != '')
        @can('checkin', \App\Models\Asset::class)
       <a href="{{ route('checkin/hardware', $asset->id) }}">{{ trans('admin/hardware/general.checkin') }}</a> 
          @endcan
      @else
       @can('checkout', \App\Models\Asset::class) 
       <a href="{{ route('checkout/hardware', $asset->id)  }}">{{ trans('admin/hardware/general.checkout') }}</a> 
          @endcan
      @endif
    @endif
      @can('update', \App\Models\Asset::class)
     <a href="{{ route('hardware.edit', $asset->id) }}">{{ trans('admin/hardware/general.edit') }}</a>
      @endcan
      @can('create', \App\Models\Asset::class)
