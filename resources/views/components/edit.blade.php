...
<!-- Component Tag -->
<div class="form-group {{ $errors->has('component_tag') ? ' has-error' : '' }}">
    <label for="component_tag" class="col-md-3 control-label">{{ trans('admin/components/table.component_tag') }}</label>
<div class="col-md-7 col-sm-12{{  (\App\Helpers\Helper::checkIfRequired($item, 'component_tag')) ? ' required' : '' }}">
    @if  ($item->id)
    <input class="form-control" type="text" name="component_tag" id="component_tag" value="{{ Input::old('component_tag', $item->component_tag) }}" />
    @else
	<input class="form-control" type="text" name="component_tag" id="component_tag" value="{{ Input::old('component_tag', \App\Models\Component::autoincrement_component()) }}">
    @endif

    {!! $errors->first('component_tag', '<span class="alert-msg"><i class="fa fa-times"></i> :message</span>') !!}
</div>
</div>
...
<div class="form-group {{ $errors->has('location_id') ? ' has-error' : '' }}">
    <label for="location_id" class="col-md-3 control-label">{{ trans('admin/hardware/form.default_location') }}</label>
    <div class="col-md-7 col-sm-11">
        {{ Form::select('location_id', $location_list , Input::old('location_id', $item->location_id), array('class'=>'select2', 'style'=>'width:100%','id'=>'rtd_location_select')) }}
...
<script>
    ...
    function fetchCustomFields() {
      ...
      if(modelid=='') {
        ...
        $.get("{{config('app.url') }}/components/models/"+modelid+"/custom_fields",{_token: "{{ csrf_token() }}"},function (data) {
