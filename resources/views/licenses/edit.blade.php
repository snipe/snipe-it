...
<div class="form-group {{ $errors->has('renewal_date') ? ' has-error' : '' }}">
    <label for="renewal_date" class="col-md-3 control-label">{{ trans('admin/licenses/form.renewal_date') }}</label>

    <div class="input-group col-md-3">
        <div class="input-group date" data-provide="datepicker" data-date-format="yyyy-mm-dd"  data-autoclose="true">
            <input type="text" class="form-control" placeholder="{{ trans('general.select_date') }}" name="renewal_date" id="renewal_date" value="{{ old('renewal_date', $item->renewal_date) }}">
            <span class="input-group-addon"><i class="fa fa-calendar" aria-hidden="true"></i></span>
        </div>
        {!! $errors->first('renewal_date', '<span class="alert-msg" aria-hidden="true"><i class="fa fa-times" aria-hidden="true"></i> :message</span>') !!}
    </div>
